...
function buildPopulated(project: typeof db.projects[0]): PopulatedProject {
    ...
    void clientId;
    void ownerId;
    return {...rest, client, owner: {id: owner.id, name: owner.name, avatarUrl: owner.avatarUrl}};
}
...
export async function GET(_req: NextRequest, {params}: {params: {id: string}}) {
    ...
    if (!project) {
        return NextResponse.json({data: null, error: {message: "Project not found", code: "NOT_FOUND"}}, {status: 404});
    }
    return NextResponse.json({data: buildPopulated(project), error: null});
}
...
export async function PATCH(req: NextRequest, {params}: {params: {id: string}}) {
    ...
    db.projects[index] = {...db.projects[index], ...parsed.data, updatedAt: new Date().toISOString()};
    ...
    return NextResponse.json({data: buildPopulated(db.projects[index]), error: null});
}
