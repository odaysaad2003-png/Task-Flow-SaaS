...
export async function GET(_req: NextRequest, {params}: {params: {id: string}}) {
    ...
    if (!client) {
        return NextResponse.json({data: null, error: {message: "العميل غير موجود", code: "NOT_FOUND"}}, {status: 404});
    }
...
export async function PATCH(req: NextRequest, {params}: {params: {id: string}}) {
    ...
    db.clients[index] = {...db.clients[index], ...parsed.data};
    return NextResponse.json({data: db.clients[index], error: null});
}
